    ...
    return (
        <ThemeProvider theme={theme} defaultMode="dark" noSsr>
                ...
                >
                    <Tab label={
                        <Badge
                            color="primary"
                            overlap="rectangular"
                            badgeContent={games?.filter(g => g.status === 'starting').length || 0}
                            showZero={false}
                        >
                            <Box sx={{pr: 1, pl: 1}}>
                    ...
                    <Tab label={
                        <Badge
                            color="primary"
                            overlap="rectangular"
                            badgeContent={games?.filter(g => g.status === 'ongoing').length || 0}
                            showZero={false}>
                            <Box sx={{pr: 1, pl: 1}}>
            ...
            <Box sx={{width: '100%', textAlign: 'center', p: 0}}>
                ...
                {
                    tabIndex === '0' && games.filter(g => g.status === 'starting').length === 0
                        ? (
                ...
                {
                    tabIndex === '1' && games.filter(g => g.status === 'ongoing').length === 0
                        ? (
                            <Alert
                                severity="info"
                                sx={{m: 2}}
            ...
            <Box sx={{width: '100%'}}>
                {
                    ...
                    && <List sx={{width: '100%'}} component="nav">
                        {games.filter(g => g.status === (tabIndex === '0' ? 'starting' : 'ongoing')).map((game, index) => (
                            <ListItemButton
                                key={index}
